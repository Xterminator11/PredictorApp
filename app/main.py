...
def body_rendering():
    ...
    with st.container():
        ...
        st.number_input(label="What will be the score posted by {}?".format(match_details.get("matches").get("home")),key="score_home",min_value=1,max_value=400,step=10)
        st.number_input(label="What will be the score posted by {}?".format(match_details.get("matches").get("away")),key="score_away",min_value=1,max_value=400,step=10)
